	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"2rpx 2rpx 2rpx 8rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.laoshiName" type="text" placeholder="老师姓名"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('laoshi','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.laoshiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('laoshi','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.laoshiPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">老师手机号</text>
                     <text style="width: 70%">{{item.laoshiPhone}}</text>
                 </view>
                 <view v-show="item.laoshiIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">老师身份证号</text>
                     <text style="width: 70%">{{item.laoshiIdNumber}}</text>
                 </view>
                 ...
                 <view v-show="item.laoshiEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">电子邮箱</text>
                     <text style="width: 70%">{{item.laoshiEmail}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('laoshi','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
            </view>
        </view>
    </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`laoshi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('laoshi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.laoshiName) {
                    searchForm['laoshiName'] = this.searchForm.laoshiName
                }

                let res = await this.$api.list(`laoshi`, searchForm);
				// 如果是第一页数据置空
